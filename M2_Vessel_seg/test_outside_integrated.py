...
def test_net(
    ...
    net_1.load_state_dict(
        torch.load(dir_checkpoint_1 + "G_best_F1_epoch.pth", map_location=device)
    )
    net_1.eval()
    net_1.to(device=device)
    net_2.load_state_dict(
        torch.load(dir_checkpoint_2 + "G_best_F1_epoch.pth", map_location=device)
    )
    net_2.eval()
    net_2.to(device=device)
    net_3.load_state_dict(
        torch.load(dir_checkpoint_3 + "G_best_F1_epoch.pth", map_location=device)
    )
    net_3.eval()
    net_3.to(device=device)
    net_4.load_state_dict(
        torch.load(dir_checkpoint_4 + "G_best_F1_epoch.pth", map_location=device)
    )
    net_4.eval()
    net_4.to(device=device)
    net_5.load_state_dict(
        torch.load(dir_checkpoint_5 + "G_best_F1_epoch.pth", map_location=device)
    )
    net_5.eval()
    net_5.to(device=device)
    net_6.load_state_dict(
        torch.load(dir_checkpoint_6 + "G_best_F1_epoch.pth", map_location=device)
    )
    net_6.eval()
    net_6.to(device=device)
    net_7.load_state_dict(
        torch.load(dir_checkpoint_7 + "G_best_F1_epoch.pth", map_location=device)
    )
    net_7.eval()
    net_7.to(device=device)
    net_8.load_state_dict(
        torch.load(dir_checkpoint_8 + "G_best_F1_epoch.pth", map_location=device)
    )
    net_8.eval()
    net_8.to(device=device)
    net_9.load_state_dict(
        torch.load(dir_checkpoint_9 + "G_best_F1_epoch.pth", map_location=device)
    )
    net_9.eval()
    net_9.to(device=device)
    net_10.load_state_dict(
        torch.load(dir_checkpoint_10 + "G_best_F1_epoch.pth", map_location=device)
    )
    net_10.eval()
